...
/** @param {NS} ns **/
import { multiscan, gainRootAccess } from "/hack/utils";
...
async function little_prep(ns, hack_target, wt, gt, reserved_RAM) {
    ...
    for (let i = 0; i < full_list.length; i++) {
        ...
        if (ns.hasRootAccess(server)) {
            await ns.scp('hack/targeted-grow.js', 'home', server);
            await ns.scp('hack/targeted-weaken.js', 'home', server);
            host_servers.push(server);
        }
    }
    let usable_RAM = 0;
    let needed_RAM = ns.getScriptRam('hack/targeted-weaken.js', 'home') * wt + ns.getScriptRam('hack/targeted-grow.js', 'home') * gt;
    let c = 1;
    ...
    for (let i = 0; i < host_servers.length; i++) {
        ...
        if (weaken_threads > 0) {
            if (server == 'home') {
                threads = Math.floor((ns.getServerMaxRam(server) - ns.getServerUsedRam(server) - reserved_RAM) / ns.getScriptRam('hack/targeted-weaken.js', 'home'));
            }
            else {
                threads = Math.floor((ns.getServerMaxRam(server) - ns.getServerUsedRam(server)) / ns.getScriptRam('hack/targeted-weaken.js', 'home'));
            }
            if (threads > weaken_threads) {
                threads = weaken_threads;
            }
            if (threads >= 1) {
                ns.exec('hack/targeted-weaken.js', server, threads, threads, hack_target);
        ...
        if (grow_threads > 0) {
            if (server == 'home') {
                threads = Math.floor((ns.getServerMaxRam(server) - ns.getServerUsedRam(server) - reserved_RAM) / ns.getScriptRam('hack/targeted-grow.js', 'home'));
            }
            else {
                threads = Math.floor((ns.getServerMaxRam(server) - ns.getServerUsedRam(server)) / ns.getScriptRam('hack/targeted-grow.js', 'home'));
            }
            if (threads > grow_threads) {
                threads = grow_threads;
            }
            if (threads >= 1) {
                ns.exec('hack/targeted-grow.js', server, threads, threads, 0, hack_target);
...
async function little_hack(ns, hack_target, weaken_threads, grow_threads, hack_threads, reserved_RAM) {
    ...
    for (let i = 0; i < full_list.length; i++) {
        const server = full_list[i];
        if (ns.hasRootAccess(server)) {
            await ns.scp('hack/targeted-hack.js', 'home', server);
            await ns.scp('hack/targeted-grow.js', 'home', server);
            await ns.scp('hack/targeted-weaken.js', 'home', server);
            host_servers.push(server);
        }
    ...
    let sec_increase;
    let startTime = Date.now();
    while (grow_threads * ns.getScriptRam('hack/targeted-grow.js', 'home') + hack_threads * ns.getScriptRam('hack/targeted-hack.js', 'home') + weaken_threads * ns.getScriptRam('hack/targeted-weaken.js', 'home') > usable_RAM - host_servers.length) {
        c += 1;
    ...
    for (let i = 0; i < host_servers.length; i++) {
        ...
        let threads = 0;
        let n = 0;
        while (ns.getServerMaxRam(server) - ns.getServerUsedRam(server) > ns.getScriptRam('hack/targeted-weaken.js', 'home')) {
            if (weaken_threads > 0) {
                if (server == 'home') {
                    threads = Math.floor((ns.getServerMaxRam(server) - ns.getServerUsedRam(server) - reserved_RAM) / ns.getScriptRam('hack/targeted-weaken.js', 'home'));
                }
                ...
                if (threads >= 1) {
                    ns.exec('hack/targeted-weaken.js', server, threads, threads, hack_target, n);
                    weaken_threads -= threads;
                    await ns.sleep(5);
                }
            }
            if (grow_threads > 0) {
                ...
                if (threads >= 1) {
                    ns.exec('hack/targeted-grow.js', server, threads, threads, ns.getWeakenTime(hack_target) - ns.getGrowTime(hack_target) - 500, hack_target, n);
                    grow_threads -= threads;
                    await ns.sleep(5);
                }
            }
            if (hack_threads > 0) {
                if (server == 'home') {
                    threads = Math.floor((ns.getServerMaxRam(server) - ns.getServerUsedRam(server) - reserved_RAM) / ns.getScriptRam('hack/targeted-hack.js', 'home'));
                }
                else {
                    threads = Math.floor((ns.getServerMaxRam(server) - ns.getServerUsedRam(server)) / ns.getScriptRam('hack/targeted-hack.js', 'home'));
                }
                if (threads > hack_threads) {
                    threads = hack_threads;
                }
                if (threads >= 1) {
                    ns.exec('hack/targeted-hack.js', server, threads, threads, ns.getWeakenTime(hack_target) - ns.getHackTime(hack_target) + 500, hack_target, n, threads);
...
export async function main(ns) {
    ...
    while (true) {
        ...
        const needed_ram = (grow_threads * ns.getScriptRam('hack/targeted-grow.js', 'home') + hack_threads * ns.getScriptRam('hack/targeted-hack.js', 'home') + weaken_threads * ns.getScriptRam('hack/targeted-weaken.js', 'home'));
        ...
        if (host_servers.length == 0) {
            ...
            let prep = 1;
            const prep_RAM = ns.getScriptRam('hack/targeted-grow.js', 'home') * gt + ns.getScriptRam('hack/targeted-weaken.js', 'home') * wt;
            ...
            if (prep) {
                if (gt > 1) {
                    ns.exec('hack/targeted-grow.js', prep_server, gt, gt, 0, little_target);
                    ns.exec('hack/targeted-weaken.js', prep_server, wt, wt, little_target);
            ...
            if (prep) {
                if (gt > 1) {
                    ns.exec('hack/targeted-grow.js', prep_server, gt, gt, 0, hack_target);
                    ns.exec('hack/targeted-weaken.js', prep_server, wt, wt, hack_target);
            ...
            for (let i = 0; i < host_servers.length; i++) {
                ...
                await ns.scp('hack/targeted-grow.js', server);
                await ns.scp('hack/targeted-hack.js', server);
                await ns.scp('hack/targeted-weaken.js', server);
                ...
                while (n > 0) {
                    if (Date.now() >= initial_time + ns.getHackTime(hack_target)) {
                        while (ns.getServerMaxRam(host_servers[k]) - ns.getServerUsedRam(host_servers[k]) < ns.getScriptRam('hack/targeted-weaken', 'home') * weaken_threads) {
                            k++;
                        ...
                        ns.exec('hack/targeted-weaken.js', host_servers[k], weaken_threads, weaken_threads, hack_target);
                        await ns.sleep(weaken_time + 20);
                        i = 0;
                        initial_time = Date.now();
                        break;
                    }
                    ns.exec('hack/targeted-weaken.js', server, weaken_threads, weaken_threads, hack_target, n);
                    ns.exec('hack/targeted-grow.js', server, grow_threads, grow_threads, grow_delay, hack_target, n);
                    ns.exec('hack/targeted-hack.js', server, hack_threads, hack_threads, hack_delay, hack_target, n);
                    await ns.sleep(3);
                    n--;
                }
                await ns.sleep(5);
